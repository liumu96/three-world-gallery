...
uniform vec2 u_resolution;
  ...
  vec4 renderRipples() {
    ...
    vec2 sample1 = gl_FragCoord.xy / u_resolution.xy;
    float ratio = u_resolution.x / u_resolution.y;
    vec2 mouse = u_mouse.xy - uv;
    
    vec4 fragcolour = texture2D(u_buffer, sample1);
    ...
    float t = texture2D(u_buffer, sample1-e.zy, 1.).x;
    float r = texture2D(u_buffer, sample1-e.xz, 1.).x;
    float b = texture2D(u_buffer, sample1+e.xz, 1.).x;
    float l = texture2D(u_buffer, sample1+e.zy, 1.).x;
  ...
  float bumpMap(vec2 uv, float height, inout vec3 colourmap) {
    ...
    shade = vec3(blur(u_buffer, sample1, ps*blurStrength));
  ...
  void main() {
    ...
    if(u_renderpass) {
      ...
      fragcolour += (texture2D(u_buffer, sample1+.03).x)*.1 - .1;
