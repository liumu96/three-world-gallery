      ...
      for (let i = 0, il = this.passes.length; i < il; i++) {
        ...
        if (pass.dependencies !== null) {
          ...
          for (let d = 0, dl = pass.dependencies.length; d < dl; d++) {
            ...
            uniforms[depVar.name].value =
              depVar.renderTargets[depVar.currentTextureIndex].texture;

            if (depVar.name !== pass.variable.name) {
              uniforms["prev_" + depVar.name].value =
                depVar.renderTargets[
                  depVar.currentTextureIndex === 0 ? 1 : 0
                ].texture;
            }
          }
        }
        ...
        this.doRenderTarget(
          pass.material,
          pass.variable.renderTargets[nextTextureIndex]
        );
