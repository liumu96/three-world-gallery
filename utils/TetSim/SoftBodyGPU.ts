...
import { MultiTargetGPUComputationRenderer } from "./MultiTargetGPUComputationRenderer";
...
export class SoftBodyGPU {
  ...
  numElems: number;
  texDim: number;
  tetPositionsArray: Float32Array;
  elemPositionsArray: Float32Array;
  inputPos: Float32Array;
  grabPos: Float32Array;
  grabId: number;
  gpuCompute: MultiTargetGPUComputationRenderer;
  pos0: THREE.DataTexture;
  vel0: THREE.DataTexture;
  invMass: THREE.DataTexture;
  invRestVolumeAndColor: THREE.DataTexture;
  elemToParticlesTable: THREE.DataTexture;
  particleToElemVertsTable: THREE.DataTexture[];
  elems0: THREE.DataTexture[];
  quats0: THREE.DataTexture;
  tetIds: number[];
  oldrestingPose!: Float32Array;
  pos: {
    name: any;
  ...
  vel: {
    ...
    minFilter: 1003;
    magFilter: 1003; // Set to vertices
    // Set to vertices
    count: any;
    currentTextureIndex: number;
  };
  ...
  xpbdIntegratePass: {
    variable: any; // Set up the 7 GPGPU Passes for each substep of the FEM Simulation
  ...
  labelMesh: THREE.Mesh<any, any, THREE.Object3DEventMap> | null = null;
  labelPlane: THREE.PlaneGeometry | undefined;
  labelMaterial: THREE.MeshBasicMaterial | undefined;
  edgeMaterial: THREE.LineBasicMaterial;
  geometry: THREE.BufferGeometry<THREE.NormalBufferAttributes>;
  edgeMesh: THREE.LineSegments<any, any>;
  visVerts: Float32Array;
  numVisVerts: number;
  visMesh: THREE.Mesh<any, THREE.MeshPhysicalMaterial, THREE.Object3DEventMap>;
  constructor(
    ...
    this.elemPositionsArray = new Float32Array(this.texDim * this.texDim * 4);
    ...
    this.gpuCompute = new MultiTargetGPUComputationRenderer(
      this.texDim,
      this.texDim,
      this.renderer
    );
    ...
    this.particleToElemVertsTable = [
      this.gpuCompute.createTexture(), // Maps from vertices back to the elems gbuffer for the scatter step
    ...
    this.elems0 = [
      this.gpuCompute.createTexture(), // Stores the original vertex positions per tet
      this.gpuCompute.createTexture(),
      this.gpuCompute.createTexture(),
      this.gpuCompute.createTexture(),
    ];
    ...
    // 1. Copy prevPos to Pos
    this.copyPrevPosPass = this.gpuCompute.addPass(
      this.prevPos,
      [this.pos],
      `
    ...
    this.xpbdIntegratePass = this.gpuCompute.addPass(
      this.pos,
      [this.vel, this.pos],
      `
            out highp vec4 pc_fragColor;
    ...
    this.xpbdIntegratePass.material.uniforms["dt"] = {
      value: this.physicsParams.dt,
    };
    ...
    this.solveElemPass = this.gpuCompute.addPass(
      this.quats,
      [this.pos, this.elems, this.quats],
      `
    ...
    this.solveElemPass.material.uniforms["elemToParticlesTable"] = {
      value: this.elemToParticlesTable,
    };
    ...
    this.gatherElemPass = this.gpuCompute.addPass(
      this.elems,
      [this.pos, this.elems, this.quats],
      `
    uniform float dt;
    ...
    this.gatherElemPass.material.uniforms["dt"] = {
      value: this.physicsParams.dt,
    };
    this.gatherElemPass.material.uniforms["elemToParticlesTable"] = {
      value: this.elemToParticlesTable,
    };
    this.gatherElemPass.material.uniforms["invRestVolume"] = {
      value: this.invRestVolumeAndColor,
    };
    this.gatherElemPass.material.uniforms["invMassTex"] = {
      value: this.invMass,
    };
    ...
    this.applyElemPass = this.gpuCompute.addPass(
      this.pos,
      [this.elems, this.pos],
      `
    out highp vec4 pc_fragColor;
    ...
    this.applyElemPass.material.uniforms["particleToElemVertsTable"] = {
      value: this.particleToElemVertsTable,
    };
    ...
    this.collisionPass = this.gpuCompute.addPass(
      this.pos,
    ...
    this.collisionPass.material.uniforms["dt"] = {
      value: this.physicsParams.dt,
    };
    this.collisionPass.material.uniforms["friction"] = {
      value: this.physicsParams.friction,
    };
    this.collisionPass.material.uniforms["grabId"] = { value: -1 };
    this.collisionPass.material.uniforms["grabPos"] = {
    ...
    this.xpbdVelocityPass = this.gpuCompute.addPass(
      this.vel,
      [this.pos, this.prevPos],
      `
    out highp vec4 pc_fragColor;
    ...
    this.xpbdVelocityPass.material.uniforms["dt"] = {
      value: this.physicsParams.dt,
    };
    this.xpbdVelocityPass.material.uniforms["gravity"] = {
      value: this.physicsParams.gravity,
    };
    ...
    if (!this.labelMesh) {
      this.labelMaterial = new THREE.MeshBasicMaterial({
        map: this.gpuCompute.getCurrentRenderTarget(this.elems).texture[0],
    ...
    this.edgeMaterial.onBeforeCompile = (shader) => {
      ...
      shader.uniforms.texturePos = {
        value: this.gpuCompute.getCurrentRenderTarget(this.pos).texture,
      }; //this.pos.texture };
    };
    ...
    let vertShaderMainColor =
      vertShaderMain.slice(0, -1) +
      `
    ...
    this.geometry.setAttribute(
      "tetWeights",
      new THREE.BufferAttribute(this.visVerts, 4)
    );
    ...
    this.visMesh.material.onBeforeCompile = (shader) => {
      ...
      shader.uniforms.textureQuat = {
        value: this.gpuCompute.getCurrentRenderTarget(this.quats).texture,
      };
      shader.uniforms.elemToParticlesTable = {
        value: this.elemToParticlesTable,
      };
    };
  ...
  initPhysics(density: number) {
    ...
    for (let i = 0; i < this.particleToElemVertsTable.length; i++) {
      this.particleToElemVertsTable[i].needsUpdate = true;
    }
    ...
    for (let i = 0; i < this.numElems; i++) {
      ...
      for (let j = 0; j < 4; j++) {
        ...
        this.elems0[j].image.data[4 * i] = this.inputPos[id * 3 + 0];
        this.elems0[j].image.data[4 * i + 1] = this.inputPos[id * 3 + 1];
        this.elems0[j].image.data[4 * i + 2] = this.inputPos[id * 3 + 2];
      }
      ...
      for (let id = 0; id < 4; id++) {
        ...
        for (let t = 0; t < this.particleToElemVertsTable.length; t++) {
          for (let c = 0; c < 4; c++) {
            if (
              this.particleToElemVertsTable[t].image.data[4 * ids[id] + c] <=
              0.0
            ) {
              this.particleToElemVertsTable[t].image.data[4 * ids[id] + c] =
                4.0 * i + id;
    ...
    for (let i = 0; i < this.invMass.image.data.length; i++) {
      if (this.invMass.image.data[i] != 0.0) {
        this.invMass.image.data[i] = 1.0 / this.invMass.image.data[i];
      }
    }
    this.invMass.needsUpdate = true;
  }
  ...
  updateVisMesh() {
    ...
    if (this.physicsParams.computeNormals) {
  ...
  vecSetZero(a: Float32Array, anr: number) {
    anr *= 3;
    a[anr++] = 0.0;
    a[anr++] = 0.0;
    a[anr] = 0.0;
...
export class GPUGrabber {
  ...
  grabDistance: number;
  active: boolean;
  physicsObject: null | SoftBodyGPU;
